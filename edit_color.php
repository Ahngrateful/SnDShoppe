...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE product_colors SET color_name = ?, yards = ?, rolls = ? WHERE color_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("siii", $color_name, $yards, $rolls, $color_id);

    if ($stmt->execute()) {
        // Show a success alert and redirect
        echo "<script>
                alert('Product color updated successfully!');
                window.location.href = 'color_list.php';
              </script>";
        exit;
    } else {
...
<body>
    ...
    <form method="POST">
        ...
        <input type="text" name="color_name" value="<?= htmlspecialchars($product['color_name']) ?>" required><br>
        <label>Yards:</label>
        <input type="number" step="0.01" name="yards" value="<?= $product['yards'] ?>" required><br>
        <label>Yards:</label>
        <input type="number" step="0.01" name="rolls" value="<?= $product['rolls'] ?>" required><br>
